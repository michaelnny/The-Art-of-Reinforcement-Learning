...
def main(argv):
    """Trains Actor-Critic agent with entropy loss.
    ...
    policy_network = ActorNet(state_dim=state_dim, action_dim=action_dim)
    policy_optimizer = torch.optim.Adam(policy_network.parameters(), lr=FLAGS.policy_lr)

    value_network = CriticNet(state_dim=state_dim)
