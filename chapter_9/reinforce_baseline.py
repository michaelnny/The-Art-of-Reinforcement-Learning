...
flags.DEFINE_float('learning_rate', 0.0002, 'Learning rate for policy network.')
flags.DEFINE_float('baseline_learning_rate', 0.0005, 'Learning rate for baseline network.')
...
class BaselineMlpNet(nn.Module):
...
class ReinforceBaselineAgent:
    """REINFORCE with baseline agent for interacting with the environment and do learning."""
    ...
    def __init__(
        ...
        self.baseline_network = baseline_network.to(device=self.device)
    ...
    def update(self, episode_sequence):
        ...
        for t in range(T):
            ...
            baseline_s_t = self.baseline_network(s_t).squeeze(-1)
            advantage = g_t - baseline_s_t

            pi_logits_s_t = self.policy_network(s_t)
            m = Categorical(logits=pi_logits_s_t)
            logprob_a_t = m.log_prob(a_t)

            value_loss = 0.5 * torch.square(advantage)

            # Negative sign to indicate we want to maximize the policy gradient objective function
            policy_loss = -(self.discount**t * advantage.detach() * logprob_a_t)
...
def main(argv):
    """Trains REINFORCE with baseline agent.
    ...
    logging.info(f'Runs REINFORCE with baseline agent on {device}')
    ...
    baseline_network = BaselineMlpNet(state_dim=state_dim)
    baseline_optimizer = torch.optim.Adam(baseline_network.parameters(), lr=FLAGS.baseline_learning_rate)

    # Create training and evaluation agent instances
    train_agent = ReinforceBaselineAgent(
