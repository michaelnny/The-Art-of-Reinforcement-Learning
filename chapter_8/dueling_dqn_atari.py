...
class DuelingDqnConvNet(nn.Module):
    ...
    def __init__(self, state_dim: tuple, action_dim: int):
        ...
        self.state_value_head = nn.Linear(512, 1)

        self.advantage_value_head = nn.Linear(512, action_dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        state_values = self.state_value_head(features)  # [batch_size, 1]
        advantage_values = self.advantage_value_head(features)  # [batch_size, action_dim]

        q_values = state_values + (
            advantage_values - torch.mean(advantage_values, dim=1, keepdim=True)
...
def main(argv):
    """Trains dueling double DQN agent.
    ...
    logging.info(f'Runs dueling double DQN agent on {device}')
    ...
    network = DuelingDqnConvNet(state_dim=state_dim, action_dim=action_dim)
    optimizer = torch.optim.Adam(
        network.parameters(),
        lr=FLAGS.learning_rate,
    )
    target_network = DuelingDqnConvNet(state_dim=state_dim, action_dim=action_dim)
