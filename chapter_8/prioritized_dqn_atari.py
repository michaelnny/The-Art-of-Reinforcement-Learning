...
flags.DEFINE_float(
    'importance_sampling_exponent_begin_value',
    0.4,
    'Importance sampling exponent begin value.',
)
flags.DEFINE_float(
    'importance_sampling_exponent_end_value',
    1.0,
    'Importance sampling exponent end value after decay.',
)
flags.DEFINE_bool(
    'normalize_weights',
    True,
...
class PrioritizedDQNAgent:
    """Double DQN agent with prioritized experience replay for interacting with the environment and do learning."""
    ...
    def act(self, observation):
        ...
        if self.step_t % self.update_frequency == 0:
            transitions, indices, weights = self.replay.sample(self.batch_size)
            priorities = self.update(transitions, weights)
            self.update_t += 1

            priorities = np.abs(priorities)
            max_priority = priorities.max()
            self.max_seen_priority = np.max([self.max_seen_priority, max_priority])
            self.replay.update_priorities(indices, priorities)

            if self.update_t % self.target_network_update_frequency == 0:
                self.update_target_network()
    ...
    def save_transition(self, transition):
        self.replay.add(transition, self.max_seen_priority)
    ...
    def update(self, transitions, weights):
        ...
        loss, priorities = self.calc_loss(transitions, weights)
        loss.backward()
    ...
    def calc_loss(self, transitions, weights):
        ...
        weights = torch.from_numpy(weights).to(device=self.device, dtype=torch.bool)  # [batch_size]
        ...
        with torch.no_grad():
            best_a_tp1 = torch.argmax(self.network(s_tp1), dim=1)
            q_tp1 = self.target_network(s_tp1)  # [batch_size, action_dim]
            max_q_tp1 = q_tp1.gather(1, best_a_tp1.unsqueeze(1)).squeeze(1)  # [batch_size]
            target_t = r_t + discount_tp1 * max_q_tp1

        # Compute loss which is 0.5 * square(td_errors)
        qa_t = q_t.gather(1, a_t.unsqueeze(1)).squeeze(1)
        td_error = target_t - qa_t
        priorities = td_error.detach().cpu().numpy()

        loss = 0.5 * td_error**2

        # Add importance sampling weights to loss, where weights don't need gradients.
        loss = weights.detach() * loss
...
def main(argv):
    """Trains prioritized double DQN agent.
    ...
    logging.info(f'Runs prioritized double DQN agent on {device}')
    ...
    # Create train and evaluation environment
    ...
    importance_sampling_exponent_schedule = utils.linear_schedule(
    ...
    train_agent = PrioritizedDQNAgent(
        network=network,
