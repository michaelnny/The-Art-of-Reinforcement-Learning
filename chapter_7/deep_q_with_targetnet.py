...
class NaiveDeepQAgent:
    ...
    def calc_loss(self, transitions):
        ...
        with torch.no_grad():
            q_tp1 = self.target_network(s_tp1)  # [1, action_dim]
...
def run_train_loop(env, agent, num_train_steps):
    ...
    for _ in range(num_train_steps):
        ...
        transition = Transition(s_t=s_t, a_t=a_t, r_t=r_t, s_tp1=s_tp1, done_tp1=done)
...
def main(argv):
    """Trains naive deep Q learning with target Q network agent.
    ...
    logging.info(f'Runs naive deep Q learning with target Q network on {device}')
    ...
    target_network = DqnMlpNet(state_dim=state_dim[0], action_dim=action_dim)
