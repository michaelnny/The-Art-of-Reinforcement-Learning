...
"""
Environment class for Maximization Bias MDP example from Figure 6.5 of book "Reinforcement Learning: An Introduction", by Sutton, Richard S. and Barto.
...
class MaximizationBiasMDPEnv:

    """A very simple environment for the Maximization Bias MDP example."""
    ...
    def __init__(self):
        ...
        self.dynamics = {
            self.state_space.a: {
                self.action_space.go_left: (0, [(1, self.state_space.b)]),
                self.action_space.go_right: (0, [(1, self.state_space.end)]),
            },
            self.state_space.b: {  # We use additional logitc in step() to dedice the rewards when select each action in state B.
    ...
    def reset(self):
        ...
        self.current_state = self.state_space.a
        self.steps = 0
    ...
    def step(self, action):
        ...
        if self.current_state == self.state_space.b and action not in (
    ...
    def is_start_state(self, state):
        return state == self.state_space.a
