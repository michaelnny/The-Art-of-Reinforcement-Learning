...
class Node:
    ...
    def child_U(self, c_puct_base: float, c_puct_init: float) -> np.ndarray:
        ...
        return pb_c * np.array(
            [child.prior * (math.sqrt(self.N) / (1 + child.N)) for child in self.children.values()],
            dtype=np.float32,
        )
...
def parallel_uct_search(
    ...
    while root_node.N < num_simulations + num_parallel:
        ...
        if leaves:
            batched_nodes, batched_obs, batched_opponent_player = map(list, zip(*leaves))
            prior_probs, values = eval_func(np.stack(batched_obs, axis=0), True)

            for leaf, prior_prob, value, opponent_player in zip(batched_nodes, prior_probs, values, batched_opponent_player):
